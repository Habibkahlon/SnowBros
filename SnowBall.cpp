...
void Snowball::update(float dt, Platform* platforms, int platformCount)
{
    ...
    for (int i = 0; i < platformCount; i++)
    {
        ...
        if (s.position.x + s.size.x > p.position.x &&   // snowballâ€™s right side is to the right of platformâ€™s left side
            s.position.x < p.position.x + p.size.x &&   // snowballâ€™s left side is to the left of platformâ€™s right side
            s.position.y + s.size.y >= p.position.y &&  // snowballâ€™s bottom is below (or touching) platformâ€™s top
            s.position.y < p.position.y + p.size.y)     // snowballâ€™s top is above platformâ€™s bottom
        {
            active = false;
            return;
        }
